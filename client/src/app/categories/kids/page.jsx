...
export default async function KidsPage() {
  const products = await getProductsServer({ category: "Kids" });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-10">
        <h1 className="font-display text-5xl text-foreground">Kids&apos; sneakers</h1>
        {visibleProducts.length > 0 ? (
          <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {visibleProducts.map((product) => <FeaturedProductCard key={product.id} product={product} />)}
          </div>
        ) : (
          <div className="mt-8 rounded-[28px] border border-dashed border-border bg-card/40 p-10 text-center text-muted-foreground">No kids&apos; or both-category shoes added yet.</div>
        )}
      </main>
      <Footer />
    </div>
